...
def broadcast_globals_to_edges_eager(graph):
    """Broadcasts the global features to the edges of a graph.
...
def broadcast_sender_nodes_to_edges_eager(graph):
    """Broadcasts the node features to the edges they are sending into.
...
def broadcast_receiver_nodes_to_edges_eager(graph):
    """Broadcasts the node features to the edges they are receiving from.
...
class EdgesToGlobalsAggregatorEager(tf.keras.Model):
    """Aggregates all edges into globals."""
    ...
    def __init__(self, reducer, name="edges_to_globals_aggregator"):
        ...
        * unsorted_segment_max_or_zero
        ...
        Args:
          ...
          name: The module name.
        """
        super(EdgesToGlobalsAggregatorEager, self).__init__(name=name)
        self._reducer = reducer
...
class NodesToGlobalsAggregatorEager(tf.keras.Model):
    """Aggregates all nodes into globals."""
    ...
    def __init__(self, reducer, name="nodes_to_globals_aggregator"):
        ...
        * unsorted_segment_max_or_zero
        ...
        Args:
          ...
          name: The module name.
        """
        super(NodesToGlobalsAggregatorEager, self).__init__(name=name)
        self._reducer = reducer
...
class _EdgesToNodesAggregator(snt.AbstractModule):
    ...
    def _build(self, graph):
        _validate_graph(graph, (EDGES, SENDERS, RECEIVERS,), additional_message="when aggregating from edges.")
...
class _EdgesToNodesAggregatorEager(tf.keras.Model):
    """Agregates sent or received edges into the corresponding nodes."""

    def __init__(self, reducer, use_sent_edges=False,
                 name="edges_to_nodes_aggregator"):
        super(_EdgesToNodesAggregatorEager, self).__init__(name=name)
        self._reducer = reducer
        self._use_sent_edges = use_sent_edges
    ...
    def __init__(self, reducer, name="sent_edges_to_nodes_aggregator"):
        ...
        * unsorted_segment_max_or_zero
        ...
        Args:
          ...
          name: The module name.
        """
        super(SentEdgesToNodesAggregator, self).__init__(use_sent_edges=True, reducer=reducer, name=name)


class SentEdgesToNodesAggregatorEager(_EdgesToNodesAggregatorEager):
    """Agregates sent edges into the corresponding sender nodes."""
    ...
    def __init__(self, reducer, name="sent_edges_to_nodes_aggregator"):
        ...
        * unsorted_segment_max_or_zero
        ...
        Args:
          ...
          name: The module name.
        """
        super(SentEdgesToNodesAggregatorEager, self).__init__(use_sent_edges=True, reducer=reducer, name=name)
    ...
    def __init__(self, reducer, name="received_edges_to_nodes_aggregator"):
        ...
        * unsorted_segment_max_or_zero
        ...
        Args:
          ...
          name: The module name.
        """
        super(ReceivedEdgesToNodesAggregator, self).__init__(use_sent_edges=False, reducer=reducer, name=name)


class ReceivedEdgesToNodesAggregatorEager(_EdgesToNodesAggregatorEager):
    """Agregates received edges into the corresponding receiver nodes."""
    ...
    def __init__(self, reducer, name="received_edges_to_nodes_aggregator"):
        ...
        * unsorted_segment_max_or_zero
        ...
        Args:
          ...
          name: The module name.
        """
        super(ReceivedEdgesToNodesAggregatorEager, self).__init__(use_sent_edges=False, reducer=reducer, name=name)
...
class EdgeBlockEager(tf.keras.Model):
    """Edge block.
    ...
    def __init__(self,
        ...
        Raises:
          ValueError: When fields that are required are missing.
        """
        super(EdgeBlockEager, self).__init__(name=name)
        ...
        self._inner_layers = list()

        for size in layer_sizes:
            self._inner_layers.append(tf.keras.layers.Dense(size, activation="tanh"))
    ...
    def call(self, graph):
        ...
        if self._use_receiver_nodes:
            edges_to_collect.append(broadcast_receiver_nodes_to_edges_eager(graph))

        if self._use_sender_nodes:
            edges_to_collect.append(broadcast_sender_nodes_to_edges_eager(graph))

        if self._use_globals:
            edges_to_collect.append(broadcast_globals_to_edges_eager(graph))

        collected_edges = tf.concat(edges_to_collect, axis=-1)

        for layer in self._inner_layers:
            collected_edges = layer(collected_edges)

        return graph.replace(edges=collected_edges)
...
class NodeBlockEager(tf.keras.Model):
    """Node block.
    ...
    def __init__(self,
        ...
        super(NodeBlockEager, self).__init__(name=name)
        ...
        if self._use_received_edges:
            if received_edges_reducer is None:
                raise ValueError(
                    "If `use_received_edges==True`, `received_edges_reducer` "
                    "should not be None.")
            self._received_edges_aggregator = ReceivedEdgesToNodesAggregatorEager(received_edges_reducer)
        if self._use_sent_edges:
            if sent_edges_reducer is None:
                raise ValueError(
                    "If `use_sent_edges==True`, `sent_edges_reducer` "
                    "should not be None.")
            self._sent_edges_aggregator = SentEdgesToNodesAggregatorEager(sent_edges_reducer)
    ...
    def call(self, graph):
        ...
        for layer in self._inner_layers:
            collected_nodes = layer(collected_nodes)

        return graph.replace(nodes=collected_nodes)
...
class GlobalBlockEager(tf.keras.Model):
    """Global block.
    ...
    def __init__(self,
        ...
        super(GlobalBlockEager, self).__init__(name=name)
        ...
        if self._use_edges:
            if edges_reducer is None:
                raise ValueError(
                    "If `use_edges==True`, `edges_reducer` should not be None.")
            self._edges_aggregator = EdgesToGlobalsAggregatorEager(edges_reducer)
        if self._use_nodes:
            if nodes_reducer is None:
                raise ValueError(
                    "If `use_nodes==True`, `nodes_reducer` should not be None.")
            self._nodes_aggregator = NodesToGlobalsAggregatorEager(nodes_reducer)
    ...
    def call(self, graph):
        ...
        for layer in self._inner_layers:
            collected_globals = layer(collected_globals)

        return graph.replace(globals=collected_globals)
