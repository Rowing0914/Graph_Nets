...
class GraphNetworkEager(tf.keras.Model):
    """Implementation of a Graph Network.
    ...
    def __init__(self,
        ...
        Args:
          ...
          name: The module name.
        """
        super(GraphNetworkEager, self).__init__(name=name)
        edge_block_opt = _make_default_edge_block_opt(edge_block_opt)
        node_block_opt = _make_default_node_block_opt(node_block_opt, reducer)
        global_block_opt = _make_default_global_block_opt(global_block_opt, reducer)

        self.internal_timestep = internal_timestep
        self._edge_block = blocks.EdgeBlockEager(layer_sizes=edge_layer_sizes, **edge_block_opt)
        self._node_block = blocks.NodeBlockEager(layer_sizes=node_layer_sizes, **node_block_opt)
        self._global_block = blocks.GlobalBlockEager(layer_sizes=global_layer_sizes, **global_block_opt)

    def call(self, inputs):
        latent_graph = inputs
        for _ in range(self.internal_timestep):
            latent_graph = self._edge_block(latent_graph)
            latent_graph = self._node_block(latent_graph)
            latent_graph = self._global_block(latent_graph)
        return latent_graph
